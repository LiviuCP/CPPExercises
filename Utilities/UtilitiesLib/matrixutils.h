/* Each CPPExercises subproject that includes matrixutils.h should have following path setup in INCLUDEPATH (relative to
   the subproject dir): 1) CPPExercises/External/Matrix/MatrixLib/Matrix 2) CPPExercises/External/Matrix/MatrixLib/Utils
*/
...
// this operator needs to be included (by including "matrixutils.h") otherwise the boolean value of the matrix will get
// printed
template <typename T> std::ostream& operator<<(std::ostream& out, const Matrix<T>& data)
{
...
/* Retrieves the row and column number of the matrix based on given diagonal index:
   ...
   - instead of Matrix it would be sufficient to put the numberr of rows and columns as arguments, but the Matrix object
   is guaranteed to have valid dimensions so it's a safer option (besides the result of the diagonal index mapping to
   coordinates is supposed to be used on a real matrix)
...
*/
template <typename T>
std::pair<MatrixPoint, std::optional<matrix_diff_t>> mapDiagonalIndexToRowAndColumnNr(const Matrix<T>& matrix,
                                                                                      matrix_diff_t diagonalIndex)
{
    ...
    if (!matrix.isEmpty())
    {
        ...
        const matrix_diff_t c_TotalElementsCount{static_cast<matrix_diff_t>(c_NrOfRows) *
        ...
        const matrix_diff_t c_ElementsCountBeforeFirstMaxDiagonal{c_FirstMaxDiagonalNr * (c_FirstMaxDiagonalNr + 1) /
                                                                  2};
        ...
        const matrix_diff_t c_ElementsCountIncludingMaxDiagonals{c_TotalElementsCount -
                                                                 c_ElementsCountBeforeFirstMaxDiagonal};
        ...
        if (diagonalIndexToConvert < c_ElementsCountBeforeFirstMaxDiagonal)
        {
            ...
            */
            ...
            const matrix_diff_t c_ElementsCountBeforeDiagonal{c_DiagonalNr > 0 ? (c_DiagonalNr * (c_DiagonalNr + 1) / 2)
                                                                               : 0};
        ...
        else if (diagonalIndexToConvert < c_ElementsCountIncludingMaxDiagonals)
        {
            ...
            const matrix_diff_t c_DiagonalNr{c_FirstMaxDiagonalNr +
                                             (*diagonalIndexToConvert - c_ElementsCountBeforeFirstMaxDiagonal) /
...
/* This function retrieves the diagonal index based on the provided coordinates (row and column number)
   - same considerations as for previous function regarding usage of Matrix and type-independent relationship between
   diagonal index and coordinates
   - similar to the previous function, for empty matrixes a null value is returned
*/
template <typename T>
std::pair<std::optional<matrix_diff_t>, MatrixPoint> mapRowAndColumnNrToDiagonalIndex(const Matrix<T>& matrix,
                                                                                      MatrixPoint rowAndColumnNr)
{
    ...
    if (!matrix.isEmpty() && rowAndColumnNr.first.has_value() && rowAndColumnNr.second.has_value())
    {
        ...
        const matrix_diff_t c_TotalElementsCount{static_cast<matrix_diff_t>(c_NrOfRows) *
                                                 static_cast<matrix_diff_t>(c_NrOfColumns)};
        const matrix_diff_t c_NrOfDiagonals{static_cast<matrix_diff_t>(c_NrOfRows) +
        ...
        {
            const matrix_diff_t c_ElementsCountBeforeFirstMaxDiagonal{c_FirstMaxDiagonalNr *
                                                                      (c_FirstMaxDiagonalNr + 1) / 2};
            const matrix_diff_t c_ElementsCountOnPrecedingMaxDiagonals{(c_DiagonalNr - c_FirstMaxDiagonalNr) *
                                                                       static_cast<matrix_diff_t>(c_MaxDiagonalSize)};
        ...
        else
        {
            ...
            const auto c_SymmetricalResult{
                mapRowAndColumnNrToDiagonalIndex(matrix, {c_SymmetricalRowNr, c_SymmetricalColumnNr})};
