...
- input files in folder InputCSVFilesExamples
- exampleoutput.csv

When building the project, these examples are copied into the build directory. When executing the application, the output CSV file (output.csv) is written onto the same directory level with the input csv directory. The exampleoutput.csv file has been provided as reference to compare the output resulting from the input examples.
...
For each input CSV file, error logging is being provided in case any parsing errors occurred. A log file with the same name as the CSV but a different suffix (.csv replaced by _error.txt) is being created next to the input file.
...
- a limit of parsed rows per file has been implemented. Currently this is 100 rows. Additional rows get discarded by parser.
