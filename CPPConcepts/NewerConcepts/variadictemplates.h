...
template <typename DataType, typename... ArgumentsList>
ValueSizePair<DataType> binaryLeftFoldMinusRecursiveImplementation(DataType initialValue,
...
template <typename DataType, typename... ArgumentsList>
ValueSizePair<DataType> binaryRightFoldMinusRecursiveImplementation(DataType initialValue,
                                                                    ArgumentsList... argumentsList);
...
template <typename DataType, typename... ArgumentsList>
std::pair<DataType, DataType> getMinMaxArgument(const DataType& first, const DataType& second,
...
template <typename DataType, typename... ArgumentsList>
ValueSizePair<DataType> binaryRightFoldMinusRecursiveImplementationHelper(DataType initialValue,
                                                                          ArgumentsList... argumentsList);
...
auto getSumOfArgumentSquares(numeric auto initialValue,
                             std::convertible_to<decltype(initialValue)> auto... argumentsList);

template <numeric... ArgumentsList> auto createTupleWithAlternatingSigns(const ArgumentsList&... argumentsList);
...
bool areTuplesEqual(const std::tuple<FirstTupleArgs...>& first, const std::tuple<SecondTupleArgs...>& second,
                    DataType epsilon);
...
template <typename DataType, typename... ArgumentsList>
ValueSizePair<DataType> Variadic::binaryLeftFoldMinusRecursiveImplementation(DataType initialValue,
                                                                             ArgumentsList... argumentsList)
{
...
template <typename DataType, typename... ArgumentsList>
ValueSizePair<DataType> Variadic::binaryRightFoldMinusRecursiveImplementation(DataType initialValue,
                                                                              ArgumentsList... argumentsList)
{
    DataType result{initialValue};
    constexpr size_t c_Count{sizeof...(argumentsList)};
    ...
    if constexpr (c_Count > 0)
    {
        ...
        {
            // match total number of arguments to be odd so the helper function can evaluate and calculate the required
            // value correctly
...
ValueSizePair<DataType> Variadic::unaryRightFoldMinusRecursiveImplementation(ArgumentsList... argumentsList)
{
    ...
    if constexpr (0 == c_Count % 2)
    {
        /* this sign correction is required because for an even number of pack arguments the called binary method
           (that uses the first argument from pack as initial value) inverts the sign of all operands (comparing to
           requirement)
        */
    ...
    {
        // the binary implementation should be called with an odd number of arguments within parameter pack so a
        // "neutral" initial value (DataType{}) should be chosen
...
// there should be at least 2 arguments for retrieving min/max value and these arguments (first, second) are the
// starting point for calculating the two values
template <typename DataType, typename... ArgumentsList>
std::pair<DataType, DataType> Variadic::getMinMaxArgument(const DataType& first, const DataType& second,
                                                          const ArgumentsList&... argumentsList)
{
...
/* This function works correctly in implementing the binary right fold minus behavior only when called with an odd
   number of argumentsList... items (initialValue not taken into account) For this reason:
    - it is not advisable to use it directly (instead wrap it into another function, like
   binaryRightFoldMinusRecursiveImplementation())
    - prior to using it the argumentsList... parameters count should be matched in order to ensure it is an odd number
   (see binaryRightFoldMinusRecursiveImplementation() for more details)
*/
template <typename DataType, typename... ArgumentsList>
ValueSizePair<DataType> Variadic::binaryRightFoldMinusRecursiveImplementationHelper(DataType initialValue,
                                                                                    ArgumentsList... argumentsList)
{
    DataType result{initialValue};
    constexpr size_t c_Count{sizeof...(argumentsList)};
...
template <hasAddOperator DataType, std::same_as<DataType>... ArgumentsList>
void Variadic::addBeforeAndAfterWithVariadicTemplateAndLambda(DataType before, DataType after,
                                                              ArgumentsList&... argumentsList)
{
    static_assert(sizeof...(argumentsList) > 0,
                  "At least one pack argument required for prepending/appending the given values!");

    auto addBeforeAndAfter{
        [before, after, &argumentsList...]() { (void(argumentsList = before + argumentsList + after), ...); }};

    addBeforeAndAfter();
}

// return type of the function is deduced from the type of the first argument
auto Variadic::getSumOfArgumentSquares(numeric auto initialValue,
                                       std::convertible_to<decltype(initialValue)> auto... argumentsList)
{
...
auto Variadic::createTupleWithAlternatingSigns(const ArgumentsList&... argumentsList)
{
    ...
    auto setAlternatingSignsToTupleElements{[&result]<std::size_t... Idx>(std::index_sequence<Idx...>) {
        ((std::get<Idx>(result) = (1 == Idx % 2) ? -std::abs(std::get<Idx>(result)) : std::abs(std::get<Idx>(result))),
         ...);
    }};
...
auto Variadic::createTupleWithCumulatedValues(DataType initialValue, const ArgumentsList&... argumentsList)
{
    ...
    if constexpr (c_ArgumentsCount > 0)
    {
        ...
        auto updateTupleWithCumulatedValues{
            [&result, &currentCumulatedValue]<std::size_t... Idx>(std::index_sequence<Idx...>) {
...
*/
template <numeric... FirstTupleArgs, numeric... SecondTupleArgs, numeric DataType>
bool Variadic::areTuplesEqual(const std::tuple<FirstTupleArgs...>& first, const std::tuple<SecondTupleArgs...>& second,
                              DataType epsilon)
{
